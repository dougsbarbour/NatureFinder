...
describe('FishSearchComponent', () => {
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      ...
      declarations: [FishSearchComponent, OrganismSearchComponent,
        RouterLinkDirectiveStub, QueryParamsDirectiveStub],
      schemas: [NO_ERRORS_SCHEMA]
    })
      .compileComponents();
    setFixture(TestBed.createComponent(FishSearchComponent));
  ...
  it('can get RouterLinks from template', () => {
    let numRouterLinks = 1 + 26 + mockConfig.allHabitats[modelName].length +
      mockConfig.allColors[modelName].length + mockConfig.allSizes[modelName].length;
    expect(routerLinks.length)
      .toBe(numRouterLinks, `should have ${numRouterLinks} routerLinks`);
    expect(routerLinks[0].stub.linkParams).toEqual([`/${modelName}`]);
  });
  it('should have top level buttons', async(() => {
    let topButtonLabels = ['Show All', 'Search By Prominent Color', 'Search By Habitat',
      ...
      'Search By Family', 'Search By Size'];
  ...
  it('can navigate to search by habitat', () => {
    ...
    let label = `find ${modelName} in a ${habitat} habitat`;
  ...
  it('can navigate to search by size', () => {
    let size = mockConfig.allSizes[modelName][0];
    testButtonPress(modelNamePlural, size.displayValue,
      {sortBy: 'commonName', size: size.queryValue});
  });
