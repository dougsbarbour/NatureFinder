...
describe('FlowerSearchComponent', () => {
  let component;
  let modelName = 'flower';
  let modelNamePlural = 'flowers';
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      ...
      declarations: [FlowerSearchComponent, OrganismSearchComponent,
        RouterLinkDirectiveStub, QueryParamsDirectiveStub],
      schemas: [NO_ERRORS_SCHEMA]
    })
      .compileComponents();
    setFixture(TestBed.createComponent(FlowerSearchComponent));
  ...
  it('can get RouterLinks from template', () => {
    let numRouterLinks = 1 + 26 + mockConfig.allHabitats[modelName].length +
      mockConfig.allColors[modelName].length + mockConfig.allBloomingPeriodMonths[modelName].length;
  ...
  it('should have top level buttons', async(() => {
    let topButtonLabels = ['Show All', 'Search By Prominent Color', 'Search By Habitat',
      ...
      'Search By Family', 'Search By Blooming Period'];
  ...
  it('should have all blooming periods', async(() => {
    let bloomingPeriodButtonLabels = mockConfig.allBloomingPeriodMonths[modelName];
    testButtonPresence(bloomingPeriodButtonLabels);
  }));
  ...
  it('can navigate to search by blooming period', () => {
    let bloomingPeriod = mockConfig.allBloomingPeriodMonths[modelName][0];
    testButtonPress(modelNamePlural, bloomingPeriod,
      {sortBy: 'commonName', bloomingPeriod: bloomingPeriod});
  });
